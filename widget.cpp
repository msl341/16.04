    ...
    , ui(new Ui::Widget)
{
    ...
    settings=new QSettings("/customer/setargument.ini", QSettings::IniFormat);//创建一个操作 INI 文件的配置对象
    settings->beginGroup("RPINFO");//将后续操作限定在 [RPINFO] 分组内
    ...
    mythread_key=new QThread(this);
    key_channel2=new key_thread_channel2;
...
//    });
      connect(key_channel2,&key_thread_channel2::first_key,this,[=](){
            this->set_first_key();
      });
      connect(key_channel2,&key_thread_channel2::second_key,this,[=](){
            this->set_second_key();
      });
      connect(key_channel2,&key_thread_channel2::third_key,this,[=](){
            this->set_third_key();
      });
      connect(key_channel2,&key_thread_channel2::fourth_key,this,[=](){
            this->set_fourth_key();
      });
      connect(key_channel2,&key_thread_channel2::fourth_key_long,this,[=](){
            this->set_fourth_key_long();
      });
      connect(key,&key_thread::insertion_key,this,[=](int flag){
          //是否有接mipi1的发送
          printf("mipi1接入\n");
    ...
    connect(socketclient, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(onError(QAbstractSocket::SocketError)));
    ...
    key_channel2->moveToThread(mythread_key);
    QObject::connect(mythread_key,SIGNAL(started()),key_channel2,SLOT(mythread_work()));
    QObject::connect(mythread_key,SIGNAL(finished()),mythread_key,SLOT(deleteLater()));
    mythread_key->start();
