...
void key_thread::mythread_work()
{
    ...
    stCfg.channel_value=1;// 通道1：仅检测MIPI1插拔
    while (1)
    {
       ioctl(sar_fd, MDRV_SARADC_SET_CHANNEL_MODE_READ_ADC, &stCfg);
       // printf("qwer 9734 stCfg.adc_value=:%d\n", stCfg.adc_value);
       // 仅保留MIPI1插拔检测逻辑
       if(INSERTION_9734KEY && mipi1_flag==0)
       {
           mipi1_flag=1;
           unmipi1_flag=1;
           emit insertion_key(mipi1_flag);
       }
       else if(INSERTION_02c10KEY && mipi1_flag==0)
       {
           mipi1_flag=2;
           unmipi1_flag=1;
           emit insertion_key(mipi1_flag);
       }
       else if(INSERTION_2740KEY && mipi1_flag==0)
       {
           mipi1_flag=3;
           unmipi1_flag=1;
           emit insertion_key(mipi1_flag);
...
// 通道2按键线程构造函数
key_thread_channel2::key_thread_channel2(QObject *parent) : QObject(parent)
{

}

// 通道2按键检测核心逻辑（复用原按键逻辑，仅修改通道号）
void key_thread_channel2::mythread_work()
{
    ADC_CONFIG_READ_ADC stCfg;
    int sar_fd = -1;
    double time_Start;
    ...
    ioctl(sar_fd, MS_SAR_INIT, 0);
    memset(&stCfg, 0, sizeof(ADC_CONFIG_READ_ADC));
    stCfg.channel_value=2;// 通道2：仅检测按键

    while (1)
    {
       ioctl(sar_fd, MDRV_SARADC_SET_CHANNEL_MODE_READ_ADC, &stCfg);
       // printf("按键 stCfg.adc_value=:%d\n", stCfg.adc_value);
       // 第一键检测（ADC:270~335）
        if (FIRST_KEY)
        {
            time_Start = (double)clock();
