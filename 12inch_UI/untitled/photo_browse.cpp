    ...
    ui(new Ui::photo_browse)
{
    ...
    connect(ui->label_delete,&Label_delete_p::clicked_1,this,[=](){
        ...
        if(photo_select_all_flag==1)
        {
            ...
            ui->label_select_all->setStyleSheet("image: url(:/tu/Photo/all.png);");
            deselect_all_false();
            ui->label_num->setText(QString("%1/%2").arg(photo_num-1).arg(photo_totality));
        }
    });
...
void photo_browse::delete_photo(int num)
{
    if(flag_USB==1)
    {
        QString filePath = QString("/mnt/USB_FLASH/Picture/%2/%1").arg(photo_name.at(num)).arg(QString::fromStdString(FolderPath));
        QFile file(filePath);
        if (file.exists())
        {
            file.remove();
    ...
    {
        QString filePath = QString("/mnt/SD_CARD/Picture/%2/%1").arg(photo_name.at(num)).arg(QString::fromStdString(FolderPath));
        QFile file(filePath);
        if (file.exists())
        {
            file.remove();
    ...
    if(photo_max!=0)
    {
        if(photo_max<6 && photo_max>0)
        {
            for(i_num=0;i_num<photo_max;i_num++)
            {
                if(flag_USB==1)
                {
                    labels.at(i_num)->setStyleSheet(QString("image: url(/mnt/USB_FLASH/Picture/%2/%1);").arg(photo_name.at(i_num)).arg(QString::fromStdString(FolderPath)));
                }
                else if(flag_USB==0 && flag_SD==1)
                {
                    labels.at(i_num)->setStyleSheet(QString("image: url(/mnt/SD_CARD/Picture/%2/%1);").arg(photo_name.at(i_num)).arg(QString::fromStdString(FolderPath)));
                }
            }
            label_all_false();
        ...
        else if(photo_max>6)
        {
            ...
            else if(photo_num>1 && (photo_num<(photo_totality-1) || photo_num==(photo_totality-1)))
            {
                for(i_num=0,p_num=(photo_num*6-6); i_num<6 && p_num<photo_num*6; i_num++,p_num++)
                {
                    if(flag_USB==1)
                    {
                        labels.at(i_num)->setStyleSheet(QString("image: url(/mnt/USB_FLASH/Picture/%2/%1);").arg(photo_name.at(p_num)).arg(QString::fromStdString(FolderPath)));
                    }
                    else if(flag_USB==0 && flag_SD==1)
                    {
                        labels.at(i_num)->setStyleSheet(QString("image: url(/mnt/SD_CARD/Picture/%2/%1);").arg(photo_name.at(p_num)).arg(QString::fromStdString(FolderPath)));
                    }
                }
                label_all_false();
                for(i_num=0; i_num<6; i_num++)
...
void photo_browse::init_photo_name()
{
    ...
    {
        //初始化数据
        QDir dir(QString("/mnt/USB_FLASH/Picture/%1/").arg(QString::fromStdString(FolderPath)));
    ...
    {
        //初始化数据
        QDir dir(QString("/mnt/SD_CARD/Picture/%1/").arg(QString::fromStdString(FolderPath)));
        QDir::Filters filters = QDir::Dirs | QDir::Files | QDir::NoDotAndDotDot;
        QDir::SortFlags sortFlags = QDir::DirsFirst | QDir::IgnoreCase | QDir::Name;
        QFileInfoList fileList = dir.entryInfoList(filters, sortFlags);
        photo_max = fileList.size();
    ...
    else if(photo_max>6)
    {
        if(photo_max % 6 != 0)
        {
            if(photo_num == photo_totality)
            {
                ...
                printf("photo_num == photo_totality\n");
            }
            else
            {
                deselect_all_true();
