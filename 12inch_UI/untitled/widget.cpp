    ...
    , ui(new Ui::Widget)
{
    ...
    threadBattery = new QThread(this);
    battery = new Battery;
    ...
    addReport_widget = new addReport(this);
    browseReport_widget = new BrowseReport(this);
    fullReport_widget = new FullReport(this);
    keyboard_widget = new KeyBoard(this);
    errorFormat_widget = new ErrorFormat(this);
    reportSaved_widget = new ReportSaved(this);
    ...
    confirm_widget = new ComfirmDelete(this);
    ...
    ui->one_stack->addWidget(addReport_widget);
    ui->one_stack->addWidget(browseReport_widget);
    ui->one_stack->addWidget(fullReport_widget);
    ...
    ui->two_stack->addWidget(confirm_widget);

    ui->two_stack->addWidget(keyboard_widget);
    ui->two_stack->addWidget(errorFormat_widget);
    ui->two_stack->addWidget(reportSaved_widget);
    ...
    connect(formatting_widget,&formatting::format_ok,this,[=](){
        ...
        if(QFile::exists("/customer/database/SD_CARD/report.db")){
            if(QFile::remove("/customer/database/SD_CARD/report.db")){
                qDebug() << "SD_CARD Database file deleted successfully.";
            }
            else {
                qDebug() << "Failed to delete SD_CARD database file.";
            }
        }
        else{
            qDebug() << "SD_CARD Database file does not exist.";
        }

//        if(QFile::exists("/customer/database/USB_FLASH/report.db")){
//            if(QFile::remove("/customer/database/USB_FLASH/report.db")){
//                qDebug() << "USB_FLASH Database file deleted successfully.";
//            }
//            else {
//                qDebug() << "Failed to delete USB_FLASH database file.";
//            }
//        }
//        else{
//            qDebug() << "USB_FLASH Database file does not exist.";
//        }
    ...
    connect(keyboard_widget, &KeyBoard::hideKeyBoard, this, [=](){
       ui->two_stack->setVisible(false);
       ui->one_stack->currentWidget() == addReport_widget ? addReport_widget->clearAllFocus() : browseReport_widget->clearAllFocus();
    });
    connect(keyboard_widget, &KeyBoard::clickPos, this, [=](QPoint pos){
       ui->two_stack->setVisible(false);
       ui->one_stack->currentWidget() == addReport_widget ? addReport_widget->setFocus(pos) : browseReport_widget->setFocus(pos);
    });
    connect(keyboard_widget, &KeyBoard::showStackTwo, this, [=]{
        ui->two_stack->setVisible(true);
    });

    connect(errorFormat_widget, &ErrorFormat::hideErrorFormat, this, [=]{
       ui->two_stack->setVisible(false);
    });

    connect(control_widget, &control_interface::clicked_browseReport, this, [=]{
        ui->one_stack->setCurrentWidget(browseReport_widget);
        browseReport_widget->chooseReport();
    });
    connect(control_widget, &control_interface::goEditReport, this, [=](int obj_id){
        addReport_widget->editReport(obj_id);
        addReport_widget->setReselectVisible(true);
        addReport_widget->setFrom(3);
        ui->one_stack->setCurrentWidget(addReport_widget);
    });

    connect(zhukuang_widget, &zhukuang::clicked_addReport, this, [=]{
        addReport_widget->getPatientID();
        ui->one_stack->setCurrentWidget(addReport_widget);
    });
    connect(zhukuang_widget, &zhukuang::clicked_browseReport, this, [=]{
        ui->one_stack->setCurrentWidget(browseReport_widget);
    });

    connect(addReport_widget, &addReport::back, this, [=](int obj_id, int where){
       if(obj_id > -1){
           browseReport_widget->addNewReport();
       }
       ...
       if(where == 0){
           ...
           fullReport_widget->restoreReport();
           ui->one_stack->setCurrentWidget(fullReport_widget);
       }
       else if(where == 2){
           ui->one_stack->setCurrentWidget(browseReport_widget);
       }
           ...
           ui->two_stack->setCurrentWidget(reportSaved_widget);
           ui->two_stack->setVisible(true);
           timer_1->start(2000);
       }
       else{
    ...
    connect(addReport_widget, &addReport::check, this, [=](QString pid, QString oid, QString name){
        setFolderPath(pid, oid);
        control_widget->setCurPatient(name);
        if(oid != ""){
            control_widget->choosedOID = oid;
            control_widget->isChoosed = true;
        }
        else{
            control_widget->choosedOID = "";
            control_widget->isChoosed = false;
        }
        ui->one_stack->setCurrentWidget(control_widget);
    });
    connect(addReport_widget, &addReport::previewClicked, this, [=](int where, int id, int age, QList<QString> photoPaths, QList<QString> others){
        fullReport_widget->previewReport(where, id, age, photoPaths, others);
        fullReport_widget->hideEdit();
        ui->one_stack->setCurrentWidget(fullReport_widget);
    });
    connect(addReport_widget, &addReport::edited, this, [=](int obj_id, bool showstatus){
        fullReport_widget->updateReport(obj_id);
        browseReport_widget->updateReport(obj_id);
//        ui->one_stack->setCurrentWidget(fullReport_widget);
//        if(control_widget->isChoosed){
//            ui->one_stack->setCurrentWidget(control_widget);
//        }
        if(showstatus){
            ui->two_stack->setCurrentWidget(reportSaved_widget);
    ...
    connect(addReport_widget, &addReport::backZhukuang, this, [=]{
        ui->one_stack->setCurrentWidget(zhukuang_widget);
    });
    connect(addReport_widget, &addReport::reselect, this, [=]{
        control_widget->isChoosed = false;
        control_widget->choosedOID = -1;
        control_widget->setCurPatient("default");
        ui->one_stack->setCurrentWidget(browseReport_widget);
        browseReport_widget->chooseReport();
        setFolderPath("default", "");
    });
    connect(addReport_widget, &addReport::showKeyBoard, this, [=]{
        ui->two_stack->setCurrentWidget(keyboard_widget);
        ui->two_stack->setVisible(true);
        if(!keyboard_widget->btnVisible()){
            ui->two_stack->setVisible(false);
            keyboard_widget->moveKeyBoard();
            addReport_widget->plainTextReClick();
        }
    });
    connect(addReport_widget, &addReport::showFullKeyBoard, this, [=]{
        keyboard_widget->showFullKeyBoard();
    });

    connect(addReport_widget, &addReport::showErrorFormat, this, [=](QString errMsg){
        errorFormat_widget->setErrroMsg(errMsg);
        ui->two_stack->setCurrentWidget(errorFormat_widget);
        ui->two_stack->setVisible(true);
    });

    connect(browseReport_widget, &BrowseReport::back, this, [=]{
        ui->one_stack->setCurrentWidget(zhukuang_widget);
    });
    connect(browseReport_widget, &BrowseReport::fullBtnClicked, this, [=](int obj_id){
        ui->one_stack->setCurrentWidget(fullReport_widget);
        fullReport_widget->viewFullReport(1, obj_id);
    });
    connect(browseReport_widget, &BrowseReport::addReportClicked, this, [=]{
        addReport_widget->getPatientID(2);
        ui->one_stack->setCurrentWidget(addReport_widget);
    });
    connect(browseReport_widget, &BrowseReport::chooseBtnClicked, this, [=](QString patient_id, QString obj_id, QString patientName){
        ui->one_stack->setCurrentWidget(control_widget);
        control_widget->setCurPatient(patientName);
        setFolderPath(patient_id, obj_id);
        if(obj_id != ""){
            control_widget->choosedOID = obj_id;
            control_widget->isChoosed = true;
        }
        else{
            control_widget->choosedOID = "";
    ...
    connect(browseReport_widget, &BrowseReport::backControlInterface, this, [=]{
        ui->one_stack->setCurrentWidget(control_widget);
    });
    connect(browseReport_widget, &BrowseReport::backZhukuang, this, [=]{
        ui->one_stack->setCurrentWidget(zhukuang_widget);
    });
    connect(browseReport_widget, &BrowseReport::showKeyBoard, this, [=]{
        ui->two_stack->setCurrentWidget(keyboard_widget);
        ui->two_stack->setVisible(true);
    });

    connect(browseReport_widget, &BrowseReport::showFullKeyboard, this, [=]{
        keyboard_widget->showFullKeyBoard();
    });
    connect(browseReport_widget, &BrowseReport::showComfirm, this, [=]{
        ui->two_stack->setCurrentWidget(confirm_widget);
        ui->two_stack->setVisible(true);
    });

    connect(confirm_widget, &ComfirmDelete::deleteBtnClicked, this, [=]{
       browseReport_widget->deleteBtnClicked(browseReport_widget->rItem);
    });
    connect(confirm_widget, &ComfirmDelete::back, this, [=]{
        ui->two_stack->setVisible(false);
    });

    connect(fullReport_widget, &FullReport::back, this, [=](int where){
        if(where == 0){
            ui->one_stack->setCurrentWidget(addReport_widget);
        }
        else if(where == 1){
    ...
    connect(fullReport_widget, &FullReport::edit, this, [=](int obj_id){
        addReport_widget->editReport(obj_id);
        ui->one_stack->setCurrentWidget(addReport_widget);
    });
    ...
    connect(control_widget,&control_interface::socket_freeze1,this,[=]{
        ...
        if(flag_USB==1)
        {
            sprintf(result, "/mnt/USB_FLASH/Picture/%s/%d%d%d%d%d%d.jpg", FolderPath.c_str(), time_num.num_year.toInt(),time_num.num_month.toUInt(),time_num.num_day.toUInt(),time_num.num_hour.toInt(),time_num.num_minute.toUInt(),time_num.num_second.toUInt());
        }
        else if(flag_USB==0 && flag_SD==1)
        {
            sprintf(result, "/mnt/SD_CARD/Picture/%s/%d%d%d%d%d%d.jpg", FolderPath.c_str(), time_num.num_year.toInt(),time_num.num_month.toUInt(),time_num.num_day.toUInt(),time_num.num_hour.toInt(),time_num.num_minute.toUInt(),time_num.num_second.toUInt());
        }
    ...
    connect(control_widget,&control_interface::socket_videoon,this,[=]{
        ...
        if(flag_USB==1)
        {
            sprintf(result, "/mnt/USB_FLASH/Videohead/%s/%d%d%d%d%d%d.jpg", FolderPath.c_str(),year_w,onth_w,day_w,hour_w,minute_w,second_w);
        }
        else if(flag_USB==0 && flag_SD==1)
        {
            sprintf(result, "/mnt/SD_CARD/Videohead/%s/%d%d%d%d%d%d.jpg", FolderPath.c_str(),year_w,onth_w,day_w,hour_w,minute_w,second_w);
        }
        socketclient->write(result,strlen(result) + 1);
    ...
    connect(video_play_widget,&video_drowse_play::socket_play_video,this,[=]{
        ...
        if(flag_USB==1)
        {
            sprintf(result, "/mnt/USB_FLASH/Video/%s/%s.mp4", FolderPath.c_str(),playback_video_name);
        }
        else if(flag_USB==0 && flag_SD==1)
        {
            sprintf(result, "/mnt/SD_CARD/Video/%s/%s.mp4", FolderPath.c_str(),playback_video_name);
        }
        socketclient->write(result,strlen(result) + 1);
        qDebug()<<"===result==="<<result;
    });
    ...
    connect(socketclient, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(onError()));
...
//    flag_USB = 1;
    ...
    battery->moveToThread(threadBattery);
    connect(threadBattery, SIGNAL(started()), battery, SLOT(batteryCheck()));
    connect(battery,SIGNAL(batteryStatus(int)), control_widget, SLOT(setBatteryStatus(int)));
    connect(battery,SIGNAL(warning()), this, SLOT(showLowBatteryWarning()));
    connect(battery, SIGNAL(charging()), control_widget, SLOT(setCharging()));
    threadBattery->start();
    ...
    system("echo 100000 >/sys/devices/soc0/soc/1f203200.pwm/pwm/pwmchip0/pwm5/period");
    ...
    system("echo 100000 >/sys/devices/soc0/soc/1f203200.pwm/pwm/pwmchip0/pwm7/period");
...
void Widget::set_language_mode()
{
    ...
    addReport_widget->refreshUI();
    zhukuang_widget->refreshUI();
    browseReport_widget->refreshUI();
    system_widget->refreshUI();
    fullReport_widget->refreshUI();
    nosdcard_wiget->refreshUI();
    confirm_widget->refreshUI();
    reportSaved_widget->refreshUI();
}
...
void Widget::connectToDatabase()
{
    ...
    if(flag_USB == 1){
        db.setDatabaseName("/customer/database/USB_FLASH/report.db");
        qDebug() << "connect to USB_FLASH database";
    }
    else if(flag_SD == 1){
        db.setDatabaseName("/customer/database/SD_CARD/report.db");
        qDebug() << "connect to SD_CARD database";
    }
    else{
        qDebug() << "no database";
        db.setDatabaseName("");  // 資料庫檔案的路徑
    }


    // 檢查是否成功開啟資料庫
    if (!db.open() || db.databaseName().isEmpty()) {
        qDebug() << "Failed to connect to the database!";
        addReport_widget->setStackWidget(false);
        browseReport_widget->showErrorConnect();
    }

    else{
        qDebug() << "Connected to the database.";
        QSqlQuery query(QSqlDatabase::database());
        QString createTableQuery = "CREATE TABLE IF NOT EXISTS report ("
        ...
        if (!query.exec(createTableQuery)) {
            qDebug() << "Error creating table report :" << query.lastError().text();
            QSqlDatabase::database().rollback();
        } else {
            qDebug() << "Table report created successfully.";
        ...
        if (!query.exec(createTableQuery)) {
            qDebug() << "Error creating table photos :" << query.lastError().text();
            QSqlDatabase::database().rollback();
        } else {
            qDebug() << "Table photos created successfully.";
            QSqlDatabase::database().commit();
        }

        addReport_widget->setStackWidget(true);
        browseReport_widget->successConnect();
    }
}

void Widget::setFolderPath(QString patient_id, QString obj_id)
{
    std::string FPath;
    FPath = (QString("%1/%2").arg(patient_id).arg(obj_id)).toStdString();

    this->FolderPath = FPath;
    photo_widget->FolderPath = FPath;
    photo_single_widget->FolderPath = FPath;
    video_widget->FolderPath = FPath;
    video_play_widget->FolderPath = FPath;
}

void Widget::switchDatabase()
{
    ui->one_stack->setCurrentWidget(zhukuang_widget);
    QSqlDatabase db = QSqlDatabase::database();
    db.close();
...
void Widget::verifyDatabasePath()
{
    QString dirPath = "/customer/database/USB_FLASH";

    QDir dir(dirPath);
    if (dir.exists())
    {
        printf("/customer/database/USB_FLASH 文件夹存在\n");
    }
    else
    {
        printf("/customer/database/USB_FLASH 文件夹不存在\n");
        system("mkdir -p /customer/database/USB_FLASH");
    }

    dirPath = "/customer/database/SD_CARD";
    dir = dirPath;

    if (dir.exists())
    {
        printf("/customer/database/SD_CARD 文件夹存在\n");
    }
    else
    {
        printf("/customer/database/SD_CARD 文件夹不存在\n");
        system("mkdir -p /customer/database/SD_CARD");
    }
    system("sync");
}

void Widget::resetChooseStatus()
{
    control_widget->choosedOID = "";
    control_widget->isChoosed = false;
    control_widget->setCurPatient("default");
...
void Widget::showLowBatteryWarning()
{
    LowBattery *lowBatWarn = new LowBattery(this);
    lowBatWarn->move(440, 275);
    lowBatWarn->refreshUI();
    lowBatWarn->show();
}
...
{
    mywidget->setFolderPath("default", "");
    mywidget->resetChooseStatus();
    if(num==SIGUSR1)
    {
        flag_SD=1;
...
void Widget::onReadyRead()
{
    ...
    else if(cStringData[0]==TAKE_VIDOE)
    {
        ...
        if(flag_USB==1)
        {
            sprintf(result, "/mnt/USB_FLASH/Video/%s/%d%d%d%d%d%d.mp4", FolderPath.c_str(), year_w, onth_w, day_w, hour_w, minute_w, second_w);
        }
        else if(flag_USB==0 && flag_SD==1)
        {
            sprintf(result, "/mnt/SD_CARD/Video/%s/%d%d%d%d%d%d.mp4", FolderPath.c_str(),year_w,onth_w,day_w,hour_w,minute_w,second_w);
        }
        socketclient->write(result,strlen(result) + 1);
    }
...
{
    QString dirPath = "/mnt/SD_CARD/Picture/default";
    QString dirPath1 = "/mnt/SD_CARD/Videohead/default";
    QString dirPath2 = "/mnt/SD_CARD/Video/default";
    QString dirPath3 = "/mnt/SD_CARD/sd_upgrade";

    QDir dir(dirPath);
    if (dir.exists())
    {
        printf("/SD_CARD/Picture/default 文件夹存在\n");
    }
    else
    {
        system("mkdir -p /mnt/SD_CARD/Picture/default");
        printf("/SD_CARD/Picture/default 文件夹不存在\n");
    }

    QDir dir1(dirPath1);
    if (dir1.exists())
    {
        printf("/SD_CARD/Videohead/default 文件夹存在\n");
    }
    else
    {
        system("mkdir -p /mnt/SD_CARD/Videohead/default");
        printf("Videohead/default 文件夹不存在\n");
    }

    QDir dir2(dirPath2);
    if (dir2.exists())
    {
        printf("/SD_CARD/Video/default 文件夹存在\n");
    }
    else
    {
        system("mkdir -p /mnt/SD_CARD/Video/default");
        printf("/SD_CARD/Video/default 文件夹不存在\n");
    }
...
{
    QString dirPath = "/mnt/USB_FLASH/Picture/default";
    QString dirPath1 = "/mnt/USB_FLASH/Videohead/default";
    QString dirPath2 = "/mnt/USB_FLASH/Video/default";

    QDir dir(dirPath);
    if (dir.exists())
    {
        printf("/USB_FLASH/Picture/default 文件夹存在\n");
    }
    else
    {
        system("mkdir -p /mnt/USB_FLASH/Picture/default");
        printf("/USB_FLASH/Picture/default 文件夹不存在\n");
    }

    QDir dir1(dirPath1);
    if (dir1.exists())
    {
        printf("/USB_FLASH/Videohead/default 文件夹存在\n");
    }
    else
    {
        system("mkdir -p /mnt/USB_FLASH/Videohead/default");
        printf("Videohead/default 文件夹不存在\n");
    }
    ...
    if (dir2.exists())
    {
        printf("/USB_FLASH/Video/default 文件夹存在\n");
    }
    else
    {
        system("mkdir -p /mnt/USB_FLASH/Video/default");
        printf("/USB_FLASH/Video/default 文件夹不存在\n");
    }
    system("sync");
}
